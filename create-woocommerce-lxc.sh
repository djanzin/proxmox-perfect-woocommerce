...
if [[ "$ENGLISH" == true ]]; then
  ...
  L_SECTION_WC="WooCommerce Configuration"
  L_SECTION_SUMMARY="Summary"
  L_SECTION_CREATE="Create LXC"
  L_SECTION_INSTALL="Install WooCommerce"
  ...
  L_PROMPT_HOSTNAME="Hostname (e.g. woocommerce)"
  ...
  L_PROMPT_TITLE="Site title [My WooCommerce Shop]"
  L_PROMPT_ADMIN_USER="WP admin username [admin]"
  L_PROMPT_PHP="PHP version (8.1/8.2/8.3/8.4/8.5) [8.3]"
  L_PROMPT_MEMORY="PHP memory limit (128M/256M/512M/1024M) [512M]"
  ...
  L_PROMPT_CURRENCY="Shop currency (EUR/USD/GBP/CHF) [EUR]"
  L_PROMPT_COUNTRY="WooCommerce base country (e.g. DE, AT, US:CA) [DE]"
  L_OS_LABEL="OS"
  ...
  L_CURRENCY_LABEL="Currency"
  L_COUNTRY_LABEL="Country"
  ...
  L_WC_INSTALLING="Installing WooCommerce inside container..."
  ...
  L_PROMPT_HOSTNAME="Hostname (z.B. woocommerce)"
  ...
  L_PROMPT_TITLE="Site-Titel [My WooCommerce Shop]"
  ...
  L_PROMPT_CURRENCY="Shop-Währung (EUR/USD/GBP/CHF) [EUR]"
  L_PROMPT_COUNTRY="WooCommerce Basisland (z.B. DE, AT, US:CA) [DE]"
  ...
  L_PMA_LABEL="phpMyAdmin"
  L_FB_LABEL="FileBrowser"
  L_CURRENCY_LABEL="Währung"
  L_COUNTRY_LABEL="Land"
  ...
  L_WC_INSTALLING="WooCommerce wird im Container installiert..."
...
CT_HOSTNAME="${CT_HOSTNAME:-woocommerce}"
...
WP_TITLE="${WP_TITLE:-My WooCommerce Shop}"
...
# Währung
read -rp "$(echo -e "${BOLD}${L_PROMPT_CURRENCY}:${RESET} ")" WC_CURRENCY
WC_CURRENCY="${WC_CURRENCY:-EUR}"

# Land
read -rp "$(echo -e "${BOLD}${L_PROMPT_COUNTRY}:${RESET} ")" WC_COUNTRY
WC_COUNTRY="${WC_COUNTRY:-DE}"
...
echo -e "  ${L_CURRENCY_LABEL}   : ${CYAN}${WC_CURRENCY}${RESET}"
echo -e "  ${L_COUNTRY_LABEL}    : ${CYAN}${WC_COUNTRY}${RESET}"
echo ""
...
CREDS_DST="/root/.wc_lxc${CT_ID}_credentials_${WP_DOMAIN}.txt"
...
echo -e "  ${BOLD}${L_SHOP_URL}:${RESET}    ${SITE_PROTO}://${WP_DOMAIN}/shop"
echo -e "  ${BOLD}${L_ADMIN_URL}:${RESET}  ${SITE_PROTO}://${WP_DOMAIN}/wp-admin/admin.php?page=wc-admin"
